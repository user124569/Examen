...
object MakePrediction {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val chivato="/opt/practicas/models/chivato"
    while (java.nio.file.Files.isRegularFile(java.nio.file.Paths.get(chivato.toString())) == false){
           print(chivato.toString())
           print ("modelo no entrenado. Esperamos...")
           Thread.sleep(10000)
    }
    java.nio.file.Files.delete(java.nio.file.Paths.get(chivato.toString()));
    print ("Modelo entrenado. Procedemos...")
    ...
    val df = spark
      .readStream
      .format("kafka")
      .option("kafka.bootstrap.servers", "miKafka:9092")//cambio brkafka
    ...
    //Cambio mongohost
    val writeConfig = WriteConfig(Map("uri" -> "mongodb://miMongo:27017/agile_data_science.flight_delay_classification_response"))
